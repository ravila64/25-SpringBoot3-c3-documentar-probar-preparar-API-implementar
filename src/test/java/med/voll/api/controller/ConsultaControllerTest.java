...
class ConsultaControllerTest {
   ...
   @WithMockUser  // vamos a simular que ya estamos logueados
   void reservar_escenario1() throws Exception {
      var response = mvc.perform(post("/consultas"))
            .andReturn().getResponse();

      assertThat(response.getStatus()).isEqualTo(HttpStatus.BAD_REQUEST.value());  // pregunta error 400
   ...
   void reservar_escenario2() throws Exception {
      var fecha = LocalDateTime.now().plusHours(1);  // hora actual + 1
      ...
      assertThat(response.getStatus()).isEqualTo(HttpStatus.OK.value());  // pregunta error
